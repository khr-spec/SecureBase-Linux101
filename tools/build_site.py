...
def page(title: str, content: str, current: str, description='', active=''):
    ...
    nav = ''.join(f'<a href="{u(dest)}"' + (' aria-current="page"' if active == key else '') + f'>{label}</a>' for key, label, dest in [
        ('home', 'Forside', 'index.html'), ('modules', 'Moduler', 'index.html#moduler'),
        ('evidence', 'Beviser', 'beviser.html'), ('deployment', 'Deployment', 'deployment.html'), ('downloads', 'Rapport', 'downloads.html')])
...
def nav_links(current):
    ...
    links += '</div><div class="nav-group"><div class="nav-label">Drift og dokumentation</div>'
    for title, target in [('Deployment', 'deployment.html'), ('VM-verifikation', 'verifikation.html'), ('Alle billedbeviser', 'beviser.html'), ('Scriptvejledning', 'scriptvejledning.html'), ('Kildekode', 'kildekode.html'), ('Downloads', 'downloads.html'), ('Testplan', 'testplan.html')]:
...
<div><div class="terminal"><div class="terminal-bar"><span class="dots"><i></i><i></i><i></i></span><span>LAB-RESULTAT / 18.09.2026</span><span>BASH</span></div><div class="terminal-body"><div class="terminal-cmd"><span>$</span> bash scripts/setup.sh --apply …</div><div class="terminal-line"><span>SSH / public key</span><b class="ok">VERIFICERET</b></div><div class="terminal-line"><span>UFW / deny incoming</span><b class="ok">AKTIV</b></div><div class="terminal-line"><span>Monitor / cron / logrotate</span><b class="ok">KONFIGURERET</b></div><div class="terminal-line"><span>Genkørsel / filændringer</span><b class="ok">0</b></div><div class="result-row"><strong>19 OK</strong><strong class="warn">1 WARN</strong><strong>0 FAIL</strong></div></div><div class="terminal-foot">DOKUMENTERET VM-TEST &nbsp;·&nbsp; IKKE LIVE MONITORERING</div></div><p class="terminal-note">Status er et resumé af den dokumenterede VM-test. <a href="verifikation.html">Se rå beviser og forklaring af advarslen →</a></p></div></div></section>
<div class="container"><div class="stats"><div class="stat"><b>06</b><span>moduler med faglige begrundelser</span></div><div class="stat"><b>103</b><span>originale billedbeviser</span></div><div class="stat"><b>18</b><span>scripts med kommentarer og kørselsvejledning</span></div><div class="stat"><b>0</b><span>ændrede filer ved genkørsel</span></div></div>
...
def code_pages():
    ...
    rows=''
    for p in CODE_FILES:
        rows+=f'<tr><td><a href="{esc(href(CODE_MAP[p],cur))}"><code>{esc(p)}</code></a></td><td>{"Historisk — installeres ikke" if "/history/" in p else "Offentlig eksempel-nøgle" if p.endswith(".pub") else "Konfigurationsskabelon" if p.startswith("config/") else "Kommenteret deployment-script"}</td></tr>'
        current=CODE_MAP[p]
...
def source_zip():
    ...
    with ZipFile(dest,'w',compression=ZIP_DEFLATED,compresslevel=9) as z:
        for p in sorted(files):
            ...
            info=ZipInfo('SecureBase-Linux101/'+relative,date_time=(2026,9,19,0,0,0))
